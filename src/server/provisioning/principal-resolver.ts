...
export interface TrustedAdministratorPrincipalResolver {
  ...
  resolveUid(uid: string): Promise<PrincipalResolutionResult>;
}
...
export function createTrustedAdministratorPrincipalResolver(
  ...
  async function resolveTrustedUid(
    uid: string,
  ): Promise<PrincipalResolutionResult> {
    try {
      ...
      const snapshot = await firestore
        .doc("provisioningAdministrators/" + uid)
        .get();
      ...
      if (parsed.data.kind === "tenant_admin") {
        ...
        if (!isTenantAdministratorAuthorizedForDirectory(parsed.data, tenant)) {
          return { ok: false, code: "forbidden" };
        }
      }
  ...
  return {
    ...
    async resolve(authorizationHeader) {
      ...
      try {
        ...
        return resolveTrustedUid(identity.uid);
      } catch (error) {
        if (isRejectedIdentity(error)) {
          return { ok: false, code: "unauthenticated" };
        }
